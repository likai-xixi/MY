...
This R-10F masterdata runtime change does not create reusable frontend components. Product category tree rendering is implemented inside the existing masterdata page. The following files are pre-existing RuoYi platform pages or dialogs that the generic component checker flags because their filenames contain generic UI words such as `Dialog`, `Form`, `Table`, or `select`.

They are not owned by `masterdata`, were not introduced by this change, and are outside the R-10F product-category runtime boundary. The exception is limited to these exact paths:
