...
This R-07 customer fund idempotency change does not create or modify customer-local reusable frontend components. The following files are pre-existing RuoYi platform/system/tool/generator pages or dialogs that the generic component checker flags because their filenames contain generic UI words such as `Dialog`, `Form`, `Table`, or `select`.

They are not introduced by this change, are outside customer fund idempotency runtime code, and should not be moved into shared components as part of this CR. The exception is limited to these exact paths:
