...
This change adds the `customer` feature and does not introduce reusable frontend components. The files below are pre-existing RuoYi system/tool module-local views that the current component heuristic classifies as reusable controls because of generic names such as `Dialog`, `Form`, or `Table`.
