...
This governance/rule-change batch does not create or modify customer-local reusable frontend components. The following files are pre-existing RuoYi platform/system/tool/generator pages or dialogs that the generic component checker flags because their filenames contain generic UI words such as `Dialog`, `Form`, `Table`, or `select`.

They are not introduced by this P0 governance gate change, are outside customer runtime code, and should not be moved into shared components as part of this CR. The exception is limited to these exact paths:
