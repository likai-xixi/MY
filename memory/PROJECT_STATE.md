...
## Status
Current dependency-maintenance record `CR-20260714T102910Z-frontend-dependency-hardening` removes the four moderate frontend findings by pinning ECharts 6.1.0 and replacing the abandoned SVG/PostCSS 5 chain with the maintained spritemap plugin 6.0.0 plus SVGO 4.0.2. The production sprite preserves all 90 icon identities with zero unsafe/missing/duplicate symbols. Cache request and chart lifecycle controllers enforce latest-response wins, unmount invalidation, stable resize, instance reuse, and idempotent cleanup. Root focused tests pass 5/5, UI tests pass 7/7, audit reports zero vulnerabilities, production build transforms 2601 modules, browser icon/cache acceptance passes without console errors, scanners report no contract change, and the complete repository gate passes 431/431 with closeout. The change is bound to committed pre-review `RV-20260714T112343Z-svg-spritemap`, and two independent final reviewers report GO with no P0-P3 finding. Backend, API, route, permission, SQL, database, customer/masterdata/system/tool business behavior, sales-order, release, and deployment remain unchanged; `beforeSalesOrder` stays blocked. The next isolated governance change must make UI tests and moderate audit explicit in CI and close the `.mjs` scanner-completeness gap before the final push. Existing databases still require masterdata V005 plus the complete validation script before future acceptance.
