...
## Status
Current dependency-maintenance record `CR-20260714T100059Z-testcontainers-dependency-alignment` removes the explicit `org.testcontainers:mysql` 1.21.3 override from `ruoyi-business/pom.xml`, leaving the existing Spring Boot 3.5.14 BOM as the single version authority. The resolved test graph moved from mixed 1.21.3/1.21.4 to four Testcontainers artifacts all at 1.21.4 with zero old-version hits. Clean Maven unit tests pass 58/58; clean MySQL 8.0.36 Testcontainers integration tests pass 2/2 and identify Testcontainers 1.21.4; all eight backend reactor modules compile; scanners report no contract change; and standalone plus complete Node gates pass 426/426. Finalization, closeout, and exact 15-file scope audit pass with zero outside, forbidden, record mismatch, or unstaged paths; independent review remains pending. Root/other POMs, Java, UI, runtime configuration, SQL, APIs, DB schema, permissions, registries, graphs, governance rules, business behavior, sales-order, release, and deployment remain unchanged; `beforeSalesOrder` stays blocked. Existing databases still require masterdata V005 plus the complete validation script before future acceptance.
