...
## Status
Current platform record `CR-20260714T084732Z-change` completes all 13 Druid pool properties required by Java in `application-prod.yml`. It is based on system-security commit `12812fdf00465d923db2a0cc84d85a4bf12da9ea`, approved by `RV-20260714T012241Z-review`, and restricted to the exact production profile plus platform/change/context/memory evidence. The dynamic property probe moved from 13 missing keys to zero; production safety passed; the configured Maven reactor packaged all eight modules; the packaged `prod` process contained no unresolved placeholders before reaching the intentionally unreachable MySQL/Druid connection boundary; and the complete repository gate passed 386/386. Reproducible commands and sanitized output are persisted in the active change runtime evidence. This is configuration binding evidence, not real production service acceptance. APIs, UI, DB schema, permissions, components, dependencies, governance tools, business modules, sales-order, release, and deployment remain unchanged; `beforeSalesOrder` stays blocked. The preceding notice record is committed as `12812fdf00465d923db2a0cc84d85a4bf12da9ea` with 5/5 focused tests, Chrome isolation evidence, a 2557-module frontend build, and 386/386 complete gate. Existing databases still require masterdata V005 plus the complete validation script before future acceptance.
