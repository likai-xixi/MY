# Project State
## Current Goal
Rebaseline the MY door-industry ERP engineering core on the locked RuoYi + Vue3 + Codex Auto Dev OS base before any sales-order implementation. Product, process, option, field, calculation, and release semantics must pass the existing change/review/phase-gate workflow.

## Status
Current governance change `CR-20260720T115859Z-r-11-engineering-core-roadmap-rebaseline` is R-11. It establishes the authoritative future-state contracts for separate product models and process-plan versions, reusable option sets/values, `SALES`/`TECH`/`SYSTEM` field ownership, immutable field-scheme versions, canonical calculation input/generic decomposition output, and the `OrderVersion -> TechnicalVersion -> CalculationSnapshot -> TechnicalReleasePackage -> ProductionReleaseVersion` trace chain. It also records a destructive migration from the current nine-resource masterdata runtime and registers `9CM 标准单开` plus `9CM 对开/分格拼接` as the first golden scenarios using one product model and two process plans. The approved multi-role review allows only governance/contract roots. `engineeringCoreReady` and `beforeSalesOrder` remain blocked; runtime migration, signed numeric golden fixtures, sales-order, production, formula, calculation-engine, and DXF runtime are not implemented. The previous CI follow-up is already on `origin/master` at `bc67c4d` and GitHub Actions run `29375968356` succeeded.
