...
## Status
Current governance record `CR-20260714T114811Z-frontend-ci-and-esm-scanner-hardening` explicitly requires exact root/frontend CI commands, the `ruoyi-business -am -Pintegration-test verify` reactor, a dedicated UI test, exactly one full moderate audit, and the production frontend build. It rejects help/version/dry-run/if-present/test-selection/wrong-module and duplicate-audit false-greens, including environment-resolved commands, local run-block assignments, dynamic subcommands/arguments, and common process-wrapper chains while preserving known repository-root audits. Local release verification resolves only the Maven executable through runtime policy with standard fallback while keeping its six stages fixed and fail-fast. Executable `.mjs` helpers participate in non-route scanners without becoming routes or component candidates; nested dependencies remain excluded while the real `ruoyi-ui/src/views/tool/build/**` source tree remains governed. Final `verify:release` passes with 480/480 Node tests, 60/60 Java tests including two MySQL/Testcontainers integrations, 7/7 UI tests, zero vulnerabilities across 360 dependencies, and a 2601-module production build; two independent adversarial reviews report no P0-P3. GitHub Actions is no longer marked deferred, but `beforeSalesOrder` remains blocked. Business runtime, frontend runtime, APIs, routes, permissions, SQL, database, graph, dependencies, lockfiles, release, and deployment remain unchanged. Evidence finalization, exact staging, commit, push, post-push governance, and remote Actions confirmation are the remaining closeout steps. Existing databases still require masterdata V005 plus the complete validation script before future acceptance.
