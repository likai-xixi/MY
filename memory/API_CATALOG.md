...
- Owner: `customer`
- Module: `customer`
- Notes: Records one unified customer deposit against `CUSTOMER_DEPOSIT`, creates a `customer_deposit_batch`, and writes a `customer_fund_flow` with related business type `CUSTOMER_DEPOSIT_BATCH`. The request body must include `idempotentKey`; R-07 stores `(biz_type, idempotent_key)` in `idempotent_request`, hashes normalized business fields instead of raw JSON, replays the original `customer_fund_flow` on same-key/same-hash `SUCCESS`, rejects same-key/different-hash requests, and rejects same-key/same-hash `PROCESSING` as still processing. Omitted `accountType` or explicit `CUSTOMER_DEPOSIT` is accepted; `SAMPLE_REBATE` or any other non-`CUSTOMER_DEPOSIT` `accountType` is rejected before account balance, batch, or flow mutation. This endpoint is入金-only: omitted `flowType` or `DEPOSIT_IN` writes `DEPOSIT_IN`; `DEPOSIT_DEDUCT`, `DEPOSIT_REFUND`, `DEPOSIT_ADJUST`, and `DEPOSIT_REVERSE` are rejected with `定金录入接口只允许入金，扣减、退款、调整、冲正请走独立资金处理流程。` Public customers are rejected with a service error because order-specific deposits belong to the future sales-order module. `CR-20260625T042041Z-change` keeps this API path unchanged while moving fund mutation to `CustomerFundService`, locking `customer_fund_account` with `selectFundAccountForUpdate`, retrying concurrent account creation after `DuplicateKeyException`, and retrying `flow_no` / `deposit_batch_no` unique-key collisions.
