...
- Owner: `customer`
- Module: `customer`
- Notes: Creates sample rebate records for real customers only and keeps sample rebate in `SAMPLE_REBATE`, separate from customer deposit. This endpoint creates `sample_rebate_record` first, then the internal service path writes the `SAMPLE_REBATE_GENERATE` fund flow; the deposit endpoint cannot be used to write `SAMPLE_REBATE`.
